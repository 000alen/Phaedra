...
class Notebook extends Component {
  constructor(props) {
    ...
    this.addWikipediaSuggestionsCell =
      this.addWikipediaSuggestionsCell.bind(this);
  ...
  getCell(pageId, cellId) {
    return this.state.notebook.pages
  ...
  removeCell(pageId, cellId, operation) {
    ...
    const pageIndex = this.indexPage(pageId);
    let notebook = { ...this.state.notebook };
    notebook.pages[pageIndex].cells = notebook.pages[pageIndex].cells.filter(
      (cell) => cell.id !== cellId
    );
  ...
  addWikipediaSummaryCell(query, pageId, operation) {
    ...
    addWikipediaSummaryCell(this.state.notebook, query, pageId).then(
      (notebook) => {
        statusBarController.hideLoading();
  ...
  addWikipediaSuggestionsCell(query, pageId, operation) {
    ...
    addWikipediaSuggestionsCell(this.state.notebook, query, pageId).then(
      (notebook) => {
        statusBarController.hideLoading();
  ...
  addWikipediaImageCell(query, pageId, operation) {
    ...
    addWikipediaImageCell(this.state.notebook, query, pageId).then(
      (notebook) => {
        statusBarController.hideLoading();
